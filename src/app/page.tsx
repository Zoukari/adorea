...
function BookingModal({ lang, onClose, sections }: { lang: Lang; onClose: () => void; sections: LiveSection[] }) {
  ...
  async function reserve() {
    ...
    try {
      ...
      setDone({ reference: out.reference || '', waUrl: `https://wa.me/${WA}?text=${encodeURIComponent(buildMsg() + ref)}` })
      setSubmitting(false)
    } catch {
        ...
        {done ? (
          <div style={{ textAlign:'center', padding:'18px 0 4px' }}>
            ...
            <a href={done.waUrl} target="_blank" rel="noopener noreferrer"
              ...
              {lang==='FR' ? 'Ouvrir WhatsApp' : lang==='EN' ? 'Open WhatsApp' : 'فتح واتساب'}
            </a>

            <button onClick={onClose} style={{ background:'transparent', border:'none', marginTop:14,
              cursor:'pointer', fontSize:11.5, color:C.taupe, textDecoration:'underline',
