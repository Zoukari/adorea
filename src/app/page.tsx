...
function BookingModal({ lang, onClose, sections }: { lang: Lang; onClose: () => void; sections: LiveSection[] }) {
  ...
  async function reserve() {
    ...
    try {
      ...
      // Réservation enregistrée : on prépare le lien WhatsApp
      ...
      <div className="modal">
        ...
        {done ? (
          <div style={{ textAlign:'center', padding:'18px 0 4px' }}>
            <div style={{ width:62, height:62, borderRadius:'50%', background:'#E8F5E9',
              display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 18px',
              fontSize:30, color:'#2E7D32' }}>✓</div>

            <h2 style={{ fontFamily:'Cormorant Garamond,serif', fontSize:26, fontWeight:300,
              color:C.noir, marginBottom:8 }}>
              {lang==='FR' ? 'Réservation enregistrée' : lang==='EN' ? 'Booking recorded' : 'تم تسجيل الحجز'}
            </h2>

            {done.reference && (
              <div style={{ display:'inline-block', padding:'6px 14px', background:'#FBF5EE',
                border:`1px solid ${C.or}`, borderRadius:100, fontSize:12, color:C.taupe,
                fontFamily:'Montserrat,sans-serif', letterSpacing:'0.08em', marginBottom:16 }}>
                {done.reference}
              </div>
            )}

            <p style={{ fontSize:12.5, color:C.taupe, lineHeight:1.75, maxWidth:'34ch',
              margin:'0 auto 22px', fontFamily:'Montserrat,sans-serif' }}>
              {hasCI
                ? (lang==='FR' ? 'Votre experte doit valider votre dossier santé avant confirmation définitive. Envoyez-nous votre demande sur WhatsApp pour accélérer le traitement.'
                   : lang==='EN' ? 'Your expert must review your health form before final confirmation. Send us your request on WhatsApp to speed things up.'
                   : 'يجب أن تراجع خبيرتك ملفك الصحي قبل التأكيد النهائي.')
                : (lang==='FR' ? 'Dernière étape : envoyez-nous le récapitulatif sur WhatsApp pour confirmer votre créneau.'
                   : lang==='EN' ? 'Last step: send us the summary on WhatsApp to confirm your slot.'
                   : 'الخطوة الأخيرة: أرسلي لنا الملخص عبر واتساب لتأكيد موعدك.')}
            </p>

            <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:9,
              fontSize:12, color:C.taupe, fontFamily:'Montserrat,sans-serif', marginBottom:16 }}>
              <span className="spin"/>
              {lang==='FR' ? 'Ouverture de WhatsApp...' : lang==='EN' ? 'Opening WhatsApp...' : 'جاري فتح واتساب...'}
            </div>

            <a href={done.waUrl}
              style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:9,
                width:'100%', padding:'15px', borderRadius:100, background:'#25D366', color:'#fff',
                textDecoration:'none', fontFamily:'Montserrat,sans-serif', fontSize:12,
              ...
              {lang==='FR' ? 'Continuer sur WhatsApp' : lang==='EN' ? 'Continue on WhatsApp' : 'المتابعة على واتساب'}
            </a>

            <div style={{ fontSize:10.5, color:C.taupe, opacity:.7, marginTop:10,
              fontFamily:'Montserrat,sans-serif' }}>
              {lang==='FR' ? "Si rien ne se passe, touchez le bouton ci-dessus."
               : lang==='EN' ? 'If nothing happens, tap the button above.'
               : 'إذا لم يحدث شيء، اضغطي على الزر أعلاه.'}
            </div>
          ...
          {step===4 && <button className="btn-or" style={{flex:1,opacity:(payMethod&&!submitting)?1:0.25,cursor:payMethod?'pointer':'not-allowed'}} onClick={reserve} disabled={!payMethod||submitting}>
            {submitting ? (
              <span style={{display:'inline-flex',alignItems:'center',gap:8}}>
                <span className="spin"/>{lang==='FR'?'Enregistrement...':'Saving...'}
              </span>
            ) : lang==='FR'?'Confirmer ma réservation':lang==='EN'?'Confirm my booking':'تأكيد الحجز'}
          </button>}
        </div>
        </>
        )}
